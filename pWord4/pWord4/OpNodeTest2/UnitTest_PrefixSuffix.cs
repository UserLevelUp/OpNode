...
namespace OpNodeTest2
{
    ...
    public class UnitTest_PrefixSuffix
    {
        ...
        [TestMethod]
        public void TestJsonSerializationWithPrefixAndSuffix()
        {
            // Test JSON serialization preserves prefix and suffix
            var ns = new NameSpace();
            ns.Prefix = "data";
            ns.Suffix = "v3";
            ns.URI_PREFIX = "http://schemas.example.com/data";
            ns.URI_SUFFIX = "http://schemas.example.com/data/v3";

            try
            {
                string jsonString = JsonConvert.SerializeObject(ns, Formatting.Indented);
                Assert.IsNotNull(jsonString);
                
                // Verify all components are in JSON
                Assert.IsTrue(jsonString.Contains("data"));
                Assert.IsTrue(jsonString.Contains("v3"));
                Assert.IsTrue(jsonString.Contains("http://schemas.example.com/data"));
                Assert.IsTrue(jsonString.Contains("http://schemas.example.com/data/v3"));
            ...
            {
                Assert.Fail($"JSON serialization with prefix and suffix failed: {ex.Message}");
            }
        }
